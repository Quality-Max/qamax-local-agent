...
// cmdReceipt implements `qmax receipt <list|show|verify> [run_id|latest]` — the
// customer-facing surface for the Exposure Receipt: "here is exactly what left
// your network, and it verifies offline."
func cmdReceipt(args []string) {
	...
	default:
		fmt.Fprintf(os.Stderr, "Usage: qmax receipt <list|show|verify> [run_id|latest]\n")
		os.Exit(1)
	}
}
...
func mustResolve(idOrLatest string) *receipt.Receipt {
